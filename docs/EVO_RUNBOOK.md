...
| orders open > 24h | **maker**: limit never traded through (conservative fills working as intended) or market data stalled — check data-health section. **taker**: every open/partial order is re-evaluated against a fresh quote each cycle (process_open_orders), so this means the price simply hasn't returned to the order's limit in 24h, not that it stopped being checked — expected occasionally for thin/cheap contracts whose exact touch price is rare, but worth a look if frequent (agents should cancel a stale thesis themselves; the order will keep sitting there otherwise) |

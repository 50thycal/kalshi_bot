...
|---|---|---|---|---|---|---|
| 2026-08-13 | **PMDIV** | cross-venue disagreement (weather) | **scoped (validating a metric already shipped to the evo DSL)** | — | **pending probe** | Tests the untested premise under `pm_divergence` (`kalshi_bot/evo/signals.py`): when Polymarket's implied distribution disagrees with Kalshi's, is Polymarket the one that's right? Gates ~2 PRs (grid-free `pm_mean_gap_f` metric + a Polymarket backtest dataset). Prior is **LOW**: the same divergence test on our NWS/ensemble forecast (`weather_validation`, 2026-08-13) returned 0%/4% forecast-better in the outer bands — bigger disagreement, *more* wrong — and every weather book was retired 2026-08-12. Probe also settles whether the reported `pm_err=9.38F` is real; the batching defect behind it is already **confirmed** (all 451,198 `polymarket_snapshots.captured_at` values hold exactly one row, so `validation.py::_pm_implied_mean`'s exact-max filter returns a single bucket). `docs/PMDIV_THESIS.md`, `scripts/pm_divergence_study.py`. |

...
August–September Experiment-OS lines (PERP-V1, MARKTANGLE-1/2, queue-aware cancel) were operator-originated,
not idea-model promotions, and are not counted here; their verdicts are in `RESEARCH_JOURNAL.md`.

Post-snapshot additions remain separate from this dated historical tally: PASSIVE-PERP
(2026-09-12), one perp microstructure census, HOLD on mixed/fee-and-control-negative
evidence; 0 promotions to a trading book. Perp microstructure addition: 1 screened, 0 passed.
...
| **UNTESTABLE — venue not ready** (data absence, not a kill) | 3 | FREEZE, COMPIN, **ECON-REACT** (only ~20 genuine econ-print settles yet) — *addendum 2026-09-12: FREEZE later built, stood down, RETIRED 2026-09-06; its metals riff METALHALT KILLED at census (settlement feed is 24/7)* |
...
|---|---|---|---|---|---|---|
| 2026-09-16 | **LIQUIDITY-INCENTIVE MM** | maker / liquidity provision (subsidised two-sided quoting) | Calvin handoff; ≤ $250–$500 capital, ≥ $1/day net bar | — | **INSTRUMENT BUILT — awaiting 14-day shadow** | Phase 0 shadow MM shipped (WS-020): genuine two-sided quotes on every active liquidity-incentive market, 3 policies × 5 tiers × 3 fill models, reward from the reconstructed scoring rules, single-leg marks and settlement, read-only ranking. No orders. Verdict after ≥ 14 observation days under the pre-registered criteria in `docs/LIQUIDITY_INCENTIVE_THESIS.md` §6. Mechanically new premise for this family (a paid subsidy, not a spread vacuum), so WIDEQUOTE's ruling-out does not apply. |
| 2026-09-13 | **SPOT-PERP-CARRY** | same-asset funding carry | Calvin selected parked idea; $2,000 primary total cash, $4,000 ceiling | — | **HOLD; C1 PASS** | Approved runner: 90 observations / 30 days each BTC and ETH, no errors, eight-hour gaps. API units/scaling, executable prices and account fees unverified; no measured return. `docs/SPOT_PERP_CARRY_CENSUS.md`. |
...
| 2026-09-12 | **PASSIVE-PERP** | perp microstructure / reversion | Calvin selected BTC/ETH passive execution; probe-first approval | 2026-09-12 | **HOLD — price screen did not survive** | BTC n=49 / ETH n=44; optimistic instant-maker returns after historical 2 bps/leg fees, before funding: -4.8359 / +0.0482 bps. Both trail the matched zero-fee control. Positive premium convergence is not outright P&L. No collector or paper promotion; `docs/PASSIVE_PERP_CENSUS.md` Results and ops `passive-perp-20260912-1`. |
| 2026-08-13 | **PMDIV** | cross-venue disagreement (weather) | **scoped (validating a metric already shipped to the evo DSL)** | 2026-08-13 | **RULING-OUT (family closed)** | Probe ran first-try on 39,740 cycles / **198 settled events** / 3 cities. **P2 KILL**: both outer divergence bands ≤50% pm-better (1% and 8%), and the gradient is monotonic — 27% → 9% → 1% as disagreement grows. **The more Polymarket disagrees with Kalshi, the more reliably Polymarket is wrong**, the identical signature the NWS forecast produced hours earlier (0%/4%). P1 FAIL (pm_err 1.29F = 1.75× Kalshi's 0.74F). P3 FAIL (+0.83¢ n=59 / −7.36¢ n=11 vs a +2¢ bar). **P0 PASS and this is the durable win**: the corrected `pm_err=1.29F` against the single-bucket artifact's **9.83F** proves `weather_validation`'s 9.38F was a code defect, not a Polymarket property — `validation.py::_pm_implied_mean` matched rows by exact `max(captured_at)` while `insert_polymarket_snapshots` stamped a distinct microsecond per row, so it always read ONE bucket. Insert-side fix shipped; column still needs re-materializing. Two PRs of planned build (grid-free `pm_mean_gap_f` + a Polymarket backtest dataset) saved by one probe. `docs/PMDIV_THESIS.md` RESULTS. |

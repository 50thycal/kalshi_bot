...
| 2026-07-25 | **XLOCK** | structural / locked arb | **scoped (structural / arb-adjacent run)** | — | **pending probe** | Re-opens the 0-for-2 structural family on a **coverage** claim, not a new belief: `kalshi_arb.scan_event` is within-event only, so `KXMVE` parlays (a literal `continue`), cross-series touch-vs-terminal containment, and unflagged candidate sets are excluded by code path rather than by null result. P1 parlay⊆leg locks, P2 touch⊇terminal locks, P3 coverage census. LOW prior (liquid MECE sets sit at Σask 0.97–0.99). Pre-committed: if P1+P2 both KILL, **the locked-arb family closes permanently**. `docs/XLOCK_THESIS.md`. |

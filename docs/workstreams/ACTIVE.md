...
|---|---|---|---|---|---|
| [WS-020](WS-020-liquidity-incentive-shadow.md) | Liquidity-incentive shadow market maker — Phase 0 instrumentation, no orders | REVIEW | Active — **RUNNING** | Shadow RUNNING on evo since 2026-09-17 12:24Z (3,939 programs, 0 errors). Phase 1a added on operator authorization: the one-sided live smoke test (<=$1/order, <=25c, 3 resting, $10 book) with its own XOS package, gates and paper twin — built, **awaiting merge and armed by nobody**. Operator: thesis §10.6 arming sequence (steps 2 and 4 are hard stops); separately, day-one check 3 before any `est_` reward figure is believed | [#415](https://github.com/50thycal/kalshi_bot/pull/415), [#416](https://github.com/50thycal/kalshi_bot/pull/416) merged |

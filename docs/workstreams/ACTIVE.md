...
| [WS-009](WS-009-livedash-load-and-selection.md) | Live-vs-paper dashboard: load cost, run selection, retired-pair landing | REVIEW | Active | Merged (#271). Confirm on the deployed livedash that first paint is seconds not half a minute and that switching pairs mid-load no longer reverts — blocked on D3: the livedash service is not reachable from the ops channel | [#271](https://github.com/50thycal/kalshi_bot/pull/271) |

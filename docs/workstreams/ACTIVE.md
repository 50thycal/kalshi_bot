...
| [WS-009](WS-009-livedash-load-and-selection.md) | Live-vs-paper dashboard: load cost, run selection, retired-pair landing | BUILDING | Active | Land the PR, then confirm on the deployed service that first paint is seconds not half a minute and that switching pairs mid-load no longer reverts | — |

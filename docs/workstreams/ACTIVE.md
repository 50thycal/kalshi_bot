...
|---|---|---|---|---|---|
| [WS-020](WS-020-liquidity-incentive-shadow.md) | Liquidity-incentive shadow market maker — Phase 0 instrumentation, no orders | REVIEW | Active | Merged (#415). Operator sends the one `env set` in WS-020 § Activation state (evo service, redeploys only the idle fleet worker); then read `liquidity_incentive_report` § COLLECTOR in the first hour | [#415 merged](https://github.com/50thycal/kalshi_bot/pull/415) |

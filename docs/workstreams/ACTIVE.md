...
**Updated:** 2026-09-07 (WS-015 completed) · **Build OS v0.12**
...
|---|---|---|---|
| [WS-015](WS-015-mmsell10-queue-aware-cancel.md) | mmsell10 queue-aware cancellation: shadow instrument + pre-registered contract | 2026-09-07 | Shipped and **running in shadow**. The frozen rule (`qac-v1-2026-09-07`) was derived from a read-only baseline — 18,143 queue samples over 408 live orders, 100% telemetry coverage — and registered at PROBE with two pre-registered gates ([#364](https://github.com/50thycal/kalshi_bot/pull/364)). The first production registration was REFUSED by Postgres (a vocabulary column overflowed; SQLite tests could not catch it), fixed with a generic schema-vs-values regression test ([#365](https://github.com/50thycal/kalshi_bot/pull/365)), and retried successfully. The shadow records decisions and cancels nothing; the baseline's own finding that every fill after 90 min was a winner is pre-registered as the cost the treatment must not pay. Evidence accrual and the gate verdict belong to Experiment OS |

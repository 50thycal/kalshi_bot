...
| [WS-010](WS-010-perp-v1-pre-registration.md) | PERP-V1: a research surface for Kalshi perpetual futures | BUILDING | Active | After merge: submit `REGISTER_PACKAGE` for `perp-v1`, then run the Probe 0 surface survey — the perp API surface is unverified from the sandbox and assumption A1 is what it tests | this PR |

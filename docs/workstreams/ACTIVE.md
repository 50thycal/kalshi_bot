...
| [WS-010](WS-010-perp-v1-pre-registration.md) | PERP-V1: a research surface for Kalshi perpetual futures | BUILDING | Active | Tape is live and accumulating (26 cycles/hr, ~145 s cadence). D4 closed: no funding source exists, so arm B `perpcarry` is BLOCKED_DATA and the race is arms A and C. Probe 2 (`scripts/perp_arm_scores.py`) is written and ops-runnable; it reports which pre-registered clauses cannot be read at all rather than substituting for them. Next: run it against the live tape (D6), and `perp-v1` is still unregistered in production (an operator `REGISTER_PACKAGE` act) | [#275](https://github.com/50thycal/kalshi_bot/pull/275), [#277](https://github.com/50thycal/kalshi_bot/pull/277), [#280](https://github.com/50thycal/kalshi_bot/pull/280), [#291](https://github.com/50thycal/kalshi_bot/pull/291), this PR |

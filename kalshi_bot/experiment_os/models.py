...
class ExperimentOsExperimentCommand(Base):
    """Receipt for one experiment-LIFECYCLE command executed at worker boot.

    A THIRD ledger, disjoint from `experiment_os_issue_commands` and
    `experiment_os_platform_commands`, and for the same reason those two are
    disjoint from each other: a ticket must not be able to arm a canary, and a
    platform revision must not be able to freeze a Version. Three tables and
    three vocabularies make that structural rather than a convention someone
    could widen later.

    `payload_json` is stored so a replay can be proven identical. It is NOT
    secret — the same bytes are committed in plaintext to a public branch — so
    payloads must be safe to disclose. Here that is easy: an envelope names a
    reviewed package and an approver, and nothing else. See the module docstring
    of `experiment_commands.py`.
    """

    __tablename__ = "experiment_os_experiment_commands"
    __table_args__ = (
        Index("ix_experiment_os_experiment_commands_status", "status", "requested_at"),
    )

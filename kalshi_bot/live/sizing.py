...
def maker_no_price(settings, metrics, no_price: int | None = None) -> int | None:
    """The NO price a mmsell-style maker entry rests at: the no-bid (join the queue), improved by
    `mmsell_live_price_offset_cents`, never paying through the no-ask. Returns None when the book
    gives us nothing to price off."""
    base = no_price if no_price is not None else metrics.best_no_bid if metrics else None
    if base is None:
        return None
    price = int(base) + max(0, int(settings.mmsell_live_price_offset_cents))
...
def order_quantity(settings, price_cents: int) -> int:
    """Contracts for one live entry: the per-order dollar cap divided by the price, floored to
    whole contracts and capped by the hard `max_order_size` guard. 0 means "too expensive to
    size" and the caller must place nothing."""
    if price_cents <= 0:
        return 0
    qty = math.floor(float(settings.live_max_order_dollars) / (price_cents / 100.0))
    return max(0, min(qty, int(settings.max_order_size)))

...
# --- the reward ledger, wired into the collector's clock (thesis §9.21) --------------------
#
# Kalshi publishes a programme's terms and never our credit against them, so a liquidity reward
# is only visible as the part of a balance change nothing else explains. These cover the
# COLLECTOR's half: that the reading happens on its own schedule, anchors correctly, persists
# what it found, and cannot take the tape down when the portfolio reads fail.


class _BalanceClient(_ReadClient):
    """`_ReadClient` plus the three portfolio reads the reward ledger makes."""

    def __init__(self, *, balance=18_651, fills=None, settlements=None, fail=False, **kw):
        super().__init__(**kw)
...
def _fresh_ledger():
    """Start each ledger test from an empty table.

    These tests share one database, and the ledger is deliberately append-only with each row
    differencing the PREVIOUS one — so rows left by a neighbouring test would silently become
    another test's anchor and make its residual meaningless."""
    with db.session_scope() as session:
        session.execute(sql_delete(m.IncentiveBalanceObservation))
        session.execute(sql_delete(m.IncentiveCollectorEvent))
        session.commit()


def _observations(settings):
    with db.session_scope() as session:
...
def test_the_first_balance_reading_anchors_without_claiming_a_residual(settings):
    """Differencing the first balance against nothing would report a credit the size of the
    whole account, so the anchor row leaves every attribution column NULL. A zero residual and
    an unmeasurable one are different claims."""
    _fresh_ledger()
    col = _state(settings, client=_BalanceClient(balance=18_651))
    col.observe_balance(T0)
    rows = _observations(settings)
    assert len(rows) == 1
...
def test_cash_that_no_trade_explains_is_persisted_as_a_residual(settings):
    """The signature of a liquidity credit: the balance rose and nothing traded."""
    _fresh_ledger()
    client = _BalanceClient(balance=18_651)
    col = _state(settings, client=client)
    col.observe_balance(T0)
    client.balance = 18_654
    out = col.observe_balance(T0 + timedelta(minutes=15))
    assert out["residual_cents"] == 3
    rows = _observations(settings)
    assert len(rows) == 2
...
def test_a_material_residual_is_announced_in_the_collector_event_stream(settings):
    ...
    col.observe_balance(T0)
    client.balance = 18_654
    col.observe_balance(T0 + timedelta(minutes=15))
    with db.session_scope() as session:
        kinds = session.execute(
            select(m.IncentiveCollectorEvent.kind)
            .where(m.IncentiveCollectorEvent.kind == c.EV_REWARD_RESIDUAL)
        ).scalars().all()
    assert kinds == [c.EV_REWARD_RESIDUAL]


def test_a_balance_change_a_fill_explains_announces_nothing(settings):
    _fresh_ledger()
    client = _BalanceClient(balance=18_651)
    col = _state(settings, client=client)
    col.observe_balance(T0)
    client.balance = 18_641
    client._fills = [{"side": "yes", "action": "buy", "count": 1,
                      "yes_price": 10, "no_price": 90}]
    out = col.observe_balance(T0 + timedelta(minutes=15))
    assert out["residual_cents"] == 0
    with db.session_scope() as session:
...
def test_a_deposit_is_recorded_but_not_announced_as_a_reward(settings):
    ...
    out = col.observe_balance(T0 + timedelta(minutes=15))
    assert out["residual_cents"] == 5_000
    assert out["presumed_transfer"] is True
    rows = _observations(settings)
...
def test_the_tick_takes_a_reading_on_its_own_slower_schedule(settings):
    """Slower than the settlement pass on purpose: each observation costs paged portfolio
    reads, and a residual only means anything over a window long enough to be credited in."""
    _fresh_ledger()
    client = _BalanceClient(balance=18_651)
    col = _state(settings, client=client, liquidity_incentive_balance_seconds=900.0)
    col.tick(T0)
    assert len(_observations(settings)) == 1
    col.tick(T0 + timedelta(seconds=60))
    assert len(_observations(settings)) == 1, "read again before its interval elapsed"
    col.tick(T0 + timedelta(seconds=901))
    assert len(_observations(settings)) == 2


def test_a_portfolio_read_that_fails_cannot_stop_the_tape(settings):
    """The measurement must never be able to take down the thing it measures."""
    _fresh_ledger()
    col = _state(settings, client=_BalanceClient(fail=True))
    col.tick(T0)
    assert _observations(settings) == []
    with db.session_scope() as session:
        errors = session.execute(
            select(m.IncentiveCollectorEvent.detail)
            .where(m.IncentiveCollectorEvent.kind == c.EV_LOOP_ERROR)
        ).scalars().all()
    assert any("balance" in (e or "") for e in errors)

...
NEW_ONLY enforcement, rather than described in a docstring. The four that matter most:
  ...
  * **the control is INSIDE the experiment.** `mmsell10` is already the control arm of
    ...
    `mmsell-anchor-vol-entry` in BLOCKED_PLATFORM. Both arms share one epoch and one platform
  ...
  * **the contract records where it DISAGREES with the merged sizing.** The mechanism was sized
    on 97 live fills from one MLB slate; replayed over 3,296 paper trades the cross-series
    contest grouping it was built for is the half that does nothing. Whoever tunes this cap next
    needs that in the contract, not in a chat log.
"""
...
def test_every_arm_shares_the_same_entry_parameters(registered, xos_session):
    ...
    # Every arm's band and ceiling are the base spec; the treatment appends ONLY its cap.
    assert books[pkg.CONTROL_ARM] == pkg.BASE_BOOK_PARAMS
    assert books[pkg.CAPPED_ARM] == f"{pkg.BASE_BOOK_PARAMS},contestcap=1"


def test_the_control_carries_no_cap_and_the_treatment_does(registered, xos_session):
    _exp, ver = _version(xos_session)
    params = {a.arm_key: (a.params_json or {}) for a in read.arms_for(xos_session, ver)}
    assert params[pkg.CONTROL_ARM]["contestcap"] is None
    assert params[pkg.CAPPED_ARM]["contestcap"] == 1


def test_the_global_flag_is_held_constant_for_the_life_of_the_version(registered, xos_session):
    """The treatment opts in per BOOK. If the global switch were flipped instead, every other
    mmsell book's selection would move at the same instant and the control would stop being one."""
    _exp, ver = _version(xos_session)
    held = " ".join(ver.held_constant_json or [])
    assert "mmsell_contest_cap_enabled" in held
...
def test_the_paper_deployment_names_two_fresh_tags(registered, xos_session):
    paper = registered["paper"]
    tags = {arm.arm_key: tag for arm, tag in read.deployment_arms(xos_session, paper)}
    assert tags == {pkg.CONTROL_ARM: "Gmmsell0", pkg.CAPPED_ARM: "Gmmsell1"}
...
def test_the_contract_records_that_the_tickets_headline_axis_was_FALSIFIED(registered,
                                                                          xos_session):
    """The single most important thing in this contract. The next person to tune this cap will
    otherwise tune the cross-series grouping, measure nothing, and conclude the correlation
    thesis is dead — when what failed is the axis the ticket named, not the thesis."""
    _exp, ver = _version(xos_session)
    probe = ver.sample_json["probe"]
    assert "falsified" in probe
    # Contest grouping alone leaves the worst day no better than the control's — the finding
    # that the merged sizing, drawn from one MLB slate, could not have seen.
    assert probe["recorded_result"]["contest_grouping_only"]["worst_day_usd"] \

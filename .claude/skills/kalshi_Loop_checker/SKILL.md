...
secondary count for data:), `c` = open positions. The books CASE surfaces **every strategy
individually** (`ELSE 'book:'||strategy`) — never lump unknown strategies into one bucket, or a
newly-appeared book (like a fable session's new paper book) stays hidden. Follow-up drill-down
queries (e.g. a theta calibration slice) are allowed when a finding needs them — read-only only.
...
- **Reconcile books against the registry first.** Read `docs/BOOK_REGISTRY.md` (`git show
  FETCH_HEAD:docs/BOOK_REGISTRY.md` off the default branch). For every `book:<tag>` the query
  returned, confirm a matching registry row (join on `tag`). **Any book with trades but no
  registry row is UNTRACKED** — a book started trading without its rationale/gate written down.
  Lead the headline with it and suggest a fable/`kalshi-strategy` session backfill the row; do not
  silently reverse-engineer it. The registry rows also give you each book's pre-registered gate.
